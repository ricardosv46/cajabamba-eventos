...
const consultaCorreo = async (email: string) => {
	const res = await fetch('https://apicajabamba.plazaticket.com/public/graphql', {
		method: 'POST',
		headers: {
...
export const FormPayNotAuth = ({ isAbono, onSubmit, errores, desabilitados, isOpen, onOpen, onClose }: Props) => {
      ...
      /> */}
			...
			<form onSubmit={handleSubmit} className='grid grid-cols-1 gap-3 mt-8 lg:grid-cols-2'>
				<div className='grid grid-cols-1 gap-6 sm:grid-cols-2'>
					...
					<div className='relative'>
						<InputFloat type='text' label='Celular' name='celular' value={values.celular} onChange={handleChange} onBlur={handleBlur} />
						{errors.celular && touched.celular ? (
							<p className='absolute px-1 text-sm leading-5 text-red-500 -bottom-5 '>{errors.celular}</p>
						) : null}
					</div>

					<div className='relative'>
						<InputFloat type='email' label='Email' name='email' value={values.email} onChange={handleChange} onBlur={handleBlur} />

						{errors.email && touched.email ? (
							<p className='absolute px-1 text-sm leading-5 text-red-500 -bottom-5 '>{errors.email}</p>
						) : null}
					</div>
					...
					<p className='text-xs leading-[11px] px-1'>
						Llene sus datos correctamente, con ellos creará un registro con su cuenta para generar sus boletos para el evento.
					</p>
